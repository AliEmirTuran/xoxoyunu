...
public class deneme {
	...
	public static void main(String[] args) {
		...
		do{
			...
			do{
				System.out.print((siraX + 1)+". Oyuncuda sıra  : ");
				deger = input.nextInt();
			}while(deger < 1 || deger > 9 || tabloKontrol(deger, oyunTahtasi) == false);
			oyunTahtasi = tabloKontrol(deger,oyunTahtasi,oyuncular[siraX]);
			siraX = (oyuncular.length) - 1 == siraX ? 0 : siraX + 1;
		}while(oyunKontrol(oyunTahtasi) && xoxKontrol(oyunTahtasi));
		oyunGoster(oyunTahtasi);
		kimKazandi(oyunTahtasi);	
	}
	public static void    kimKazandi(char[][] x){
